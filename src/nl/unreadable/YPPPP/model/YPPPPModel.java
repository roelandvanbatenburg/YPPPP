...
public class YPPPPModel {
	...
	/**
	 * @param me true when the damage from the player, false when from the opponent
	 *           is requested
	 * @return string containing the damage, (max) when applicable and the max
	 *         health
	 */
	public String getDamage(boolean me) {
		return oneDigit.format(getSimpleDamage(me)) + (getSimpleDamage(me) >= getFullHealth(me) ? "(max)" : "") + " / "
				+ oneDigit.format((me ? getFullHealth(myShip) : getFullHealth(oppShip)));
	}
	...
	private YPPPPShip readShip(NodeList lst, int item) {
		...
		return new YPPPPShip((readXMLItem(lst, item, "name")), new Double(readXMLItem(lst, item, "cb")),
				new Double(readXMLItem(lst, item, "ram")), new Double(readXMLItem(lst, item, "sf")),
				new Double(readXMLItem(lst, item, "sink")),
	...
	private String readXMLItem(NodeList lst, int item, String name) {
		...
		if (node.getNodeType() == Node.ELEMENT_NODE)
			return ((name.equals("name") ? shipName.format(item) : "")
					+ ((Node) ((Element) ((Element) node).getElementsByTagName(name).item(0)).getChildNodes().item(0))
							.getNodeValue());
		shipDataError = true;
		return null;
	}
